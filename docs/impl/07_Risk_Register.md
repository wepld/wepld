...
| Risk | L | I | Trigger | Mitigation |
| --- | --- | --- | --- | --- |
| **Model edit path escapes the worktree** | M | H | a builder edit uses `..`, an absolute path, a symlink, a **hard link to an outside inode**, or a concurrent path swap | edit paths are validated `WorkspaceRelativePath` (Component-based, not substring) **and** written through a handle-relative, no-follow capability boundary: the destination entry is inspected unopened (symlink/dir/FIFO/socket/device refused) and replaced by temp-file + atomic `renameat` — the **entry** moves to a new inode, an existing inode is never written through, so a shared hard-link target outside the worktree keeps its content; non-Unix fails closed ([[10_V0_Governance_Safety_and_Limits]] §5) |
...
| **Partial edit application on a failed batch** | L | M | edit *k* is valid; edit *k+1* fails prevalidation or fails at runtime | prevalidation failures write nothing (whole batch checked before the first write); a **runtime** failure may leave earlier edits only inside the isolated attempt worktree, which is then durably Failed, never snapshotted, never promoted to a proposal ref or lesson, destroyed on the synchronous path, and never offered for rerun (Contract B, proven by lifecycle test); crash-orphaned worktrees are inert but not GC'd in V0 |
